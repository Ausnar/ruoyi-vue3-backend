-- 历史手工空壳单位清理执行脚本（prod）
-- 固定范围：正式库 2026-07-10 只读预览确认的 3 棵空壳树，共 34 个节点。
-- 根节点：217 河北科技工程职业技术大学、218 邯郸机场集团有限公司、232 山东高速服开集团。
...
BEGIN
    ...
    WHERE r.dept_source = 'manual'
      AND r.del_flag = '0'
      AND (
          (r.dept_id = 217 AND r.dept_name = '河北科技工程职业技术大学')
          OR (r.dept_id = 218 AND r.dept_name = '邯郸机场集团有限公司')
          OR (r.dept_id = 232 AND r.dept_name = '山东高速服开集团')
      );
